...
def dashboard(request):
    ...
    context = {
       'total_buku': Buku.objects.count(),
        'total_judul': Buku.objects.values('judul').distinct().count(),
        'sedang_dipinjam': Peminjaman.objects.filter(status='Dipinjam').count(),
        'sudah_dikembalikan': Peminjaman.objects.filter(status='Dikembalikan').count(),
        'dipinjam': Peminjaman.objects.filter(status='Dipinjam').count(),
        'kembali': Peminjaman.objects.filter(status='Dikembalikan').count(),
...
def hapus_buku(request, pk):
    ...
    if request.method == 'POST':
        buku.delete()
        return redirect('list_buku')
    return render(request, 'buku/hapus_confirmasi.html', {'buku': buku, 'segment': 'buku'})
